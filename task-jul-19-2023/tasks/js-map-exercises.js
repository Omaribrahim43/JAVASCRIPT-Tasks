...
function capitalizeNames(arr){
  let capitalizedArray = arr.map(name => {
...
function readyToPutInTheDOM(arr){
  return arr.map(ele => `<h1>${ele.name}</h1>` + `<h2>${ele.age}</h2>`);
}
document.write(readyToPutInTheDOM(people));
...
console.log(doubleValues([1,-2,-3]));
  ...
  */
  ...
  console.log(valTimesIndex([1,2,3,6,8]));
  console.log(valTimesIndex([1,-2,-3,1,4]));
  /*
  * Exercise 9:
